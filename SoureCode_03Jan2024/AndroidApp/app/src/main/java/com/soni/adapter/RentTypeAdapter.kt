...
class RentTypeAdapter(val context: Context, private var onItemClicked: ((pos:Int) -> Unit)):
    RecyclerView.Adapter<RentTypeAdapter.InquiriesHomeAdapterViewHoldes>() {
    ...
    override fun getItemCount(): Int {
        return  SoniApp.propertyType.size
    }
    ...
    override fun onBindViewHolder(holder: InquiriesHomeAdapterViewHoldes, position: Int) {
        ...
        }else if (position==SoniApp.propertyType.size-1){
            val params = LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.WRAP_CONTENT,
                LinearLayout.LayoutParams.WRAP_CONTENT
            )
        ...
        holder.tv_type.text =SoniApp.propertyType[position].name
