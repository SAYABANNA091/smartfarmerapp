...
class FavProductListAdapter(
    ...
    RecyclerView.Adapter<FavProductListAdapter.InquiriesHomeAdapterViewHoldes>() {
...
//        val tv_type = itemView.findViewById<TextView>(R.id.tv_type)
        ...
        val tv_product_desc = itemView.findViewById<TextView>(R.id.tv_product_desc)
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): InquiriesHomeAdapterViewHoldes {

        val view = LayoutInflater.from(parent.context).inflate(R.layout.layout_fav_product_list_adapter,parent,false)
    ...
    override fun onBindViewHolder(holder: InquiriesHomeAdapterViewHoldes, position: Int) {
        ...
        holder.tv_product_price.text="Rs."+item.price.toString().split(".")[0]
        holder.tv_product_desc.text=item.description
