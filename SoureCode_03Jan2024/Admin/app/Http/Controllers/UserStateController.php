...
class UserStateController extends Controller
{
    ...
    {
        return view('pages.User.states');
    }

    public function statesData(Request $request)
    {
        if ($request->ajax()) {
            $data = UserState::where('deleted_at', null)->orderBy('name', 'ASC')->get();
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    // $btn .= '<a title="Edit" href="'. route('addStates', base64_encode($row->id)) .'" class="close-icon" data-val="block"><svg width="15" height="15" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    $btn .= '<a title="Remove" href="'. route('deleteState', base64_encode($row->id)) .'" data-toggle="modal" data-target="#State" class="close-icon delete-confirm" data-val="remove"> <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    public function deleteState($id){
        $id = base64_decode($id);
        $UserDistrictsdata = UserDistrict::select('id')->where('states_id', $id)->where('deleted_at', null)->get();
        $UserTalukasdata = UserTaluka::select('id')->where('states_id', $id)->where('deleted_at', null)->get();
        foreach ($UserDistrictsdata as $value) {
            UserDistrict::where('id', $value['id'])->delete();
            foreach ($UserTalukasdata as $value) {
                UserTaluka::where('id', $value['id'])->delete();
            }
        }
        UserState::where('id', $id)->delete();
        return redirect()->intended('/statesList')->with('alert-success', 'States deleted successfully!');
    }
    
    public function addStates(Request $request, $id)
    {
        $id = base64_decode($id);
        $stateName = UserState::where('id', $id)->first();
        return view('pages.User.addStates', compact('id', 'stateName'));
    }

    public function addEditStates(Request $request, $id)
    {
        $id = base64_decode($id);
        $this->validate(
            $request,
        ...
        if ($id == 0) {
            $statesData = array();
            $statesData['name'] = $request->name;
            $saveStates = UserState::create($statesData);
            $msg = 'States Added Successfully!';
        }else{
            $statesData = array();
            $statesData['name'] = $request->name;
            $saveStates = UserState::where('id', $id)->update($statesData);
            $msg = 'States Updated Successfully!';
        }
            if ($saveStates) {
                return redirect()->intended('/statesList')->with('alert-success', $msg);
            }
    }
}