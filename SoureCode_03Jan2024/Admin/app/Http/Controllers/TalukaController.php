...
class TalukaController extends Controller
{
    ...
    {
        return view('pages.User.taluka');
    }
    
    public function talukaData(Request $request)
    {
        ini_set('memory_limit', '-1');
        if ($request->ajax()) {
            $data = UserTaluka::where('deleted_at', null)->with('district', 'state')->get();
            return DataTables::of($data)
                ...
                ->addColumn('districts_id', function ($row) {
                    $btn = isset($row->district->name) ? $row->district->name : '-';
                    return $btn;
                })
                ->addColumn('action', function ($row) {
                    $btn = '<div>';

                    $btn .= '<a title="Edit" href="'. route('addTaluka', base64_encode($row->id)) .'" class="close-icon" data-val="block"><svg width="15" height="15" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    $btn .= '<a title="Remove" href="'. route('deleteTaluka', base64_encode($row->id)) .'" data-toggle="modal" data-target="#Taluka" class="close-icon delete-confirm" data-val="remove"> <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    public function deleteTaluka($id){
        $id = base64_decode($id);
        UserTaluka::where('id', $id)->delete();
        return redirect()->intended('/talukaList')->with('alert-success', 'Taluka deleted successfully!');
    }
    
    public function addTaluka(Request $request, $id)
    {
        $id = base64_decode($id);
        $UserDistrictdata = UserDistrict::select('*')->orderBy('name', 'asc')->get();
        $UserStatesdata = UserState::select('*')->where('deleted_at', null)->orderBy('name', 'asc')->get();
        $UserTalukadata = UserTaluka::select('*')->where('id', $id)->where('deleted_at', null)->orderBy('name', 'asc')->first();
        return view('pages.User.addEditTaluka', compact('id', 'UserDistrictdata', 'UserTalukadata', 'UserStatesdata'));
    }

    public function addEditTaluka(Request $request, $id)
    {  
        $id = base64_decode($id);
        $this->validate(
            $request,
            ...
            [
                'name.required' => 'Please enter taluka name.',
                'districts_id.required' => 'Please enter districts.',
                'states_id.required' => 'Please enter states.'
            ]
        );
        if ($id == 0) {
            $talukaData = array();
            $talukaData['name'] = $request->name;
            $talukaData['districts_id'] = $request->districts_id;
            $talukaData['states_id'] = $request->states_id;
            $savetaluka = UserTaluka::create($talukaData);
            $msg = 'Taluka Added Successfully!';
        }
        else{ 
            $talukaData = array();
            $talukaData['name'] = $request->name;
            $talukaData['districts_id'] = $request->districts_id;
            $talukaData['states_id'] = $request->states_id;
            $savetaluka = UserTaluka::where('id', $id)->update($talukaData);
            $msg = 'Taluka Updated Successfully!';
        }
            if ($savetaluka) {
                return redirect()->intended('/talukaList')->with('alert-success', $msg);
            }
    }

    public function getDistricts(Request $request)
    { 
        $districts = UserDistrict::where('states_id', $request->state_id)->get();
